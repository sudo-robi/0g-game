        ...
        {[
          ...
          <button
            ...
            onClick={() => onSwitch(m.id)}
              className={`relative px-5 sm:px-8 py-3 sm:py-4 rounded-xl font-display font-semibold transition-all duration-300 ${
                mode === m.id
                  ? m.id === 'kids'
            ...
            >
              <span className="block text-sm sm:text-base md:text-lg">{m.label}</span>
              <span className="block text-xs sm:text-sm opacity-70">{m.sub}</span>
          </button>
        ))}
      </div>
    </div>
