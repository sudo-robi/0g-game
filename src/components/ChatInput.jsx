...
export default function ChatInput({ mode, theme, onSend, isLoading }) {
  ...
  return (
    <form onSubmit={handleSubmit} className="w-full">
      <label
        htmlFor="chat-input"
        className={`block text-sm sm:text-base font-display font-semibold mb-2 ${
      ...
      <div className="flex gap-2 sm:gap-3">
        <input
          ...
          disabled={isLoading}
          className={`flex-1 rounded-xl px-4 sm:px-5 py-3 sm:py-4 text-sm sm:text-base outline-none transition-all ${
            isKids
        ...
        <button
          ...
          className={`px-5 sm:px-8 py-3 sm:py-4 rounded-xl font-display font-bold text-sm sm:text-base transition-all disabled:opacity-40 ${
            isKids
